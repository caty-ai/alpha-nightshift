...
`make test`はすべてのスイートを実行し、最後に整合性確認の行——`suites: declared=N executed=M skipped=K`——を出力します。これにより、こっそり消えたスイートは隠れる代わりに実行全体を失敗させます。実行環境の前提条件（例えばLinux上でのmacOSの`sandbox-exec`）が満たされていないスイートは、理由を明示した上でスキップされ、無言でスキップされることはありません。
...
- ガードスキャンのスイートは、契約で定められたパスに固定バージョンのgitleaksバイナリが必要です。スイートは期待する正確なパスとバージョンを出力し、存在しない場合は理由を添えてスキップします。
...
- **約束より証明** — テストスイートがガードの挙動を固定し、CIがUbuntuとmacOSの両方で、すべてのプルリクエストごとにそれを再証明します
...
| [DESIGN.md](DESIGN.md) | 興味のある人向け | レビュー履歴を含む、オリジナルの設計ドキュメント |
