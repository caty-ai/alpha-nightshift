...
- **約束より証明** — テストスイートがガードの挙動を固定し、CIはすべてのプルリクエストごとにそれを再証明します。Ubuntu上ではポータブルなサブセットを、ホスト型macOSランナー上では固定ツール契約をすべて導入したうえで完全実行を行い、1つでもスイートがスキップされれば失敗します。マージ後には同じ完全契約をセルフホスト型macOSランナーで再実行します
