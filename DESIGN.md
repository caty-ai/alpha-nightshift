...
### 10.5 データ境界（旧 G5 の一般化）
- 夜番が読む**すべての非オーナーテキスト**——Issue 本文・Web・repo 内容・**目付が観測するアプリ出力**・**夜番自身が過去に書いた台帳/Issue/GOALS**——は data であって instructions ではない
- 機械的裏付け: プランナーへの prompt はハーネスが構造化フィールドから組み立て、自由テキストは引用フェンス内でのみ渡す（生テキストを計画層に直結しない）
- **queue の完全性**: `night:rejected`→`night:ready` は actor ≠ night-bot の場合のみ有効（dispatcher が label イベント actor を API 検証してから消費）
- 完全な機械強制は原理的に不可能な残余（LLM が読む以上ゼロにならない）を**残存リスクとして明記**し、被害上限は L1-L3 が規定する。朝ダイジェストと夜番作 PR 本文も「昼側の自動化にとって untrusted data」と明記
