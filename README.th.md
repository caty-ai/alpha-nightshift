...
`make test` จะรันทุกชุดเทสต์และจบด้วยบรรทัดสรุปยืนยัน — `suites: declared=N executed=M skipped=K` — เพื่อให้ชุดเทสต์ที่หายไปเงียบๆ ทำให้การรันล้มเหลวแทนที่จะซ่อนตัวไป ชุดเทสต์ที่สภาพแวดล้อมตามสัญญาไม่มีอยู่ในเครื่องของคุณ (เช่น `sandbox-exec` ของ macOS บน Linux) จะถูกข้ามไปพร้อมพิมพ์เหตุผลออกมา ไม่มีการข้ามแบบเงียบๆ
...
- ชุดเทสต์ guard-scan ต้องการไบนารี gitleaks ที่ถูกตรึงเวอร์ชันไว้ที่ path ตามสัญญา ชุดเทสต์จะพิมพ์ path และเวอร์ชันที่คาดหวังไว้อย่างชัดเจน และข้ามพร้อมเหตุผลถ้าไม่พบ
...
- **หลักฐานสำคัญกว่าคำสัญญา** — ชุดเทสต์ตรึงพฤติกรรมของด่านป้องกันไว้ และ CI พิสูจน์ซ้ำทุกครั้งในทุก pull request ทั้งบน Ubuntu และ macOS
...
| [DESIGN.md](DESIGN.md) | ผู้ที่อยากรู้อยากเห็น | เอกสารการออกแบบต้นฉบับพร้อมประวัติการรีวิว |
