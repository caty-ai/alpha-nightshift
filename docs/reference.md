...
|---|---|---|
| `GLM_KEY_FILE` | Required for GLM | Absolute path to a regular non-symlink file that is not group- or world-readable. Line 1 is either a bare key or `NAME=value` (no `export`, no quotes). NAME must match `^[A-Z][A-Z0-9_]*$`; the nonempty key uses only letters, digits, `.`, `_`, and `-`. |
