...
| 設定 | 既定値 | 意味 |
|---|---|---|
| `GLM_KEY_FILE` | GLM使用時に必須 | グループ・その他ユーザーが読み取れない、シンボリックリンクではない通常ファイルの絶対パス。1行目はキー単体または`NAME=value`形式にします（`export`と引用符は禁止）。NAMEは`^[A-Z][A-Z0-9_]*$`に一致し、空でないキーには英数字と`.`、`_`、`-`のみ使用できます。 |
