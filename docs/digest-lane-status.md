...
- Status: **v1.1 — frozen after L1-9 upstream review r1** (3 heterogeneous seats; §13). v1.0 → v1.1 changes are listed in §14.
...
- **`HOME` is quarantined; `GH_CONFIG_DIR` is the single credential carve-out** (D-3): the reporter authenticates to GitHub through `gh`, whose file-based auth lives in the operator's `gh` config directory. Passing that one directory (read-write — `gh` may refresh its own files) keeps `.ssh`, `.netrc`, `.aws`, `.gnupg`, `.git-credentials` out of reach, unlike a full `HOME` passthrough. Consequences documented in `docs/reference.md`: env-token auth (`GH_TOKEN` / `GITHUB_TOKEN`) is **not** passed (a reporter relying on it fails visibly as `unavailable (exit N)`); `PATH` has no version-manager shim directories, so `LANE_STATUS_CMD` should be an absolute path to an operator-controlled wrapper that sets up its own interpreter. Descendant survivors after the sweep are logged WARN and do not change the verdict.
- Outcome precedence (a run can match several rows; the first applicable wins): process inspection unavailable → timeout → `rc = 127` (`command not found`) → other non-zero rc (`exit <rc>`) → output too large → empty output → contract steps 3–8.
...
- Each of §8 rows 1–6 (timeout: `LANE_STATUS_TIMEOUT_SEC=1`, reporter sleeps 5; expected wall-clock ≈ 1 s deadline + launch window, assert generously) and row 8 (unwritable night dir): the exact section and footer lines, digest exit 0, no `lane-status.json`, stderr excerpt present in `state/logs/digest-<night>.log` and absent from the digest.
- Too large: a reporter that writes > 2 MiB → `unavailable (output too large)`, stdout file size ≤ 2 MiB (the `ulimit` bound held), digest exit 0.
...
Rejected: none. Kept against one seat: D-4 fail-closed config (Grok m9 preferred `unavailable (config: …)`; Opus and GLM endorsed the `OC_REPORT_MAX_AGE_DAYS` precedent; kept, explicitly outside Done when (3)). Quorum after rev.: 2 × GO-WITH-CHANGES + 1 × NO-GO whose every concrete scenario is closed by v1.1 — the NO-GO seat re-reviews the delta before implementation (L1-3 round-1 reviewer completes).
