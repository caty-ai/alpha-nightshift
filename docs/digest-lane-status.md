...
7. **bash 3.2 / POSIX userland**: no `timeout(1)`, no associative arrays, no `mapfile`; positional parameters beyond `$9` are written braced (`${11}`, `${12}`), never as `$11`; `jq` is the only parser. Fractional `sleep` is already relied upon by `lib/budget.sh`.
