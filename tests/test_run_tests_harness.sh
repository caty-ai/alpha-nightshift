...
assert_contains 'Tests: 2 passed, 2 failed' "$tmp/output"
assert_contains 'suites: declared=4 executed=4 skipped=0' "$tmp/output"
...
rm "$tmp/tests/test_b.sh" "$tmp/tests/test_c.sh"
printf '2\n' > "$tmp/tests/expected_suite_count"
status=0
/bin/bash "$tmp/tests/run_tests.sh" > "$tmp/output" 2>&1 || status=$?
